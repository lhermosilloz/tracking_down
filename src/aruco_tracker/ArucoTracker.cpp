...
ArucoTrackerNode::ArucoTrackerNode() : Node("aruco_tracker_node") {
    ...
    // Subs
    _image_sub = this->create_subscription<sensor_msgs::msg::Image>("/camera", qos, std::bind(&ArucoTrackerNode::image_callback, this, std::placeholders::_1));
    _camera_info_sub = this->create_subscription<sensor_msgs::msg::CameraInfo>("/camera_info", qos, std::bind(&ArucoTrackerNode::camera_info_callback, this, std::placeholders::_1));

    // Pubs
    _image_pub = this->create_publisher<sensor_msgs::msg::Image>("/image_proc", qos);
    _target_pose_pub = this->create_publisher<geometry_msgs::msg::PoseStamped>("/target_pose", qos);
}
    ...
    try {
        ...
        if (!_camera_matrix.empty() && !_dist_coeffs.empty()) {
            ...
            for (size_t i = 0; i < ids.size(); i++) {
                ...
                // Calc marker size from camera intrinsics
...
void ArucoTrackerNode::camera_info_callback(const sensor_msgs::msg::CameraInfo::SharedPtr msg) {
    ...
    // Update the camera matrix and the distortion coefficients
    _camera_matrix = cv::Mat(3, 3, CV_64F, const_cast<double*>(msg->k.data())).clone();     // Use clone to get a deep copy
    _dist_coeffs = cv::Mat(msg->d.size(), 1, CV_64F, const_cast<double*>(msg->d.data())).clone();

    // Log the first row of camera matrix to verify values
