...
void PrecisionLand::targetPoseCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
    if (_search_started) {
        ...
        // Save tag poisition/orientation in NED world frame
        _tag = getTagWorld(tag);

        // Publish world-frame tag pose for vis
...
PrecisionLand::ArucoTag PrecisionLand::getTagWorld(const ArucoTag& tag) {
    ...
    Eigen::Affine3d camera_transform = Eigen::Translation3d(0, 0, -0.1) * quat_NED; // Camera is 10 cm below drone (Change depending on how its mounted)
...
void PrecisionLand::onDeactivate() {
    // No need to do anything here since the mode will be deactivated when we land
}
...
void PrecisionLand::updateSetpoint(float dt_s) {
    ...
    if (target_lost && !_target_lost_prev) {
        ...
        RCLCPP_INFO(_node.get_logger(), "Target found: State %s", stateName(_state).c_str());
    }
    ...
    switch (_state) {
        ...
        case State::Search: {
            ...
            // Schmoove through the search waypoints
            ...
            if (positionReached(waypoint_position)) {
                ...
                // If all waypoints have been searched, start again
...
Eigen::Vector2f PrecisionLand::calculateVelocitySetpointXY() {
    ...
    // 0.1 m/s min vel and 3 m/s max vel
...
void PrecisionLand::generateSearchWaypoints() {
    ...
    // The search waypoints are generated in NED frame
    ...
    for (int layer = 0; layer < num_layers; ++layer) {
        ...
        // Reverse teh layer waypoints for the inward spiral
        std::reverse(layer_waypoints.begin(), layer_waypoints.end());

        // Adjust the z-coords for inward spiral
        for (auto& waypoint : layer_waypoints) {
            waypoint.z() = current_z;
        }

        // Push reversed waypoints to main waypoints vector
        waypoints.insert(waypoints.end(), layer_waypoints.begin(), layer_waypoints.end());

        // Decrease altitude for next outward spiral
        current_z += layer_spacing;
    }
